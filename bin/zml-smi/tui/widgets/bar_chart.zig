...
pub fn renderTo(self: *const BarChart, arena: std.mem.Allocator, ctx: vxfw.DrawContext, surface: *vxfw.Surface) std.mem.Allocator.Error!void {
    ...
    while (bucket_idx < self.buckets.len and bar_col < size.width) : (bucket_idx += 1) {
        ...
        // std.log.warn("bar height: {d}, percentage: {d} -> area: {d} -> full_rows: {d} -> empty: {d} ", .{ bar_height, bucket.percentage, bucket_area, full_rows, empty_height });

        const color = theme.colorForPercent(bucket.percentage);

        // Draw empty part (spaces) at the top
        ...
        if (remainder > 0) {
            ...
            surface.writeCell(bar_col + remainder, row_idx, .{
                .char = .{ .grapheme = "    "[4 - remainder ..], .width = 4 - remainder },
                .style = .{ .fg = color, .bg = .default },
            });
            // std.log.warn("progress 3", .{});
            row_idx += 1;
        }

        // std.log.warn("progress 4: {d}", .{row_idx});
